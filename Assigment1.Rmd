...
In this article, kan Nishida explains how we can retrieve data from web pages that are not shared as files from csv,excel or other format. The rvest package makes it easy to extract data from websites that are also problematic in R, Phyton or other programming languages. One can use the rvest package to extract the data in a specific part of an html document. To do this, Nashida specifies that we can use the SelectorGadget chrome extension to extract the portion of data that we want to import in the html file.
